...
import com.robotreg.flowapp.config.EmbeddedKafka;
import com.robotreg.flowapp.config.KafkaSseConsumer;
import com.robotreg.flowapp.config.KafkaSseProducer;
...
class FlowrowsKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/flowrows-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/flowrows-kafka/consume")
