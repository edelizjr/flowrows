...
@RestController
@RequestMapping("/api/flowrows-kafka")
public class FlowrowsKafkaResource {

    private final Logger log = LoggerFactory.getLogger(FlowrowsKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public FlowrowsKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
